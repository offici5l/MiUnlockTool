...
- Random data e.g. `ebdbf1635ecc3899c74190f9a3b2c1d4` is the value of `RAND` (not fixed , a new value is generated every time the device reboots, which is why the `deviceToken` changes each time, and this is what makes `encryptData` expire immediately after a reboot)
