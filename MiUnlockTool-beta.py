...
def retrieve_api_data(api_url):
    ...
    nonce = run("/api/v2/nonce", {"r": ''.join(random.choices("abcdefghijklmnopqrstuvwxyz", k=16)), "sid": sid})["nonce"]

    data = {
        "appId": "1", 
        "data": b64encode(f'{{"clientId": "2", "clientVersion": "7.6.727.43", "language": "en", "operate": "unlock", "pcId": "{hashlib.md5(wb_id.encode()).hexdigest()}", "product": "{product}", "region": "", "deviceInfo": {{"boardVersion": "", "product": "{product}", "socId": "", "deviceName": ""}}, "deviceToken": "{token}"}}'.encode()),
        "nonce": nonce,
        "sid": sid
    }

    return run(api_url, data)

c = retrieve_api_data("/api/v2/unlock/device/clear")
...
r = retrieve_api_data("/api/v3/ahaUnlock")
